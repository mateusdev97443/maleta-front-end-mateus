...
Este capítulo ensina separar buscar, renderizar e tratar erro no contexto de JavaScript vanilla para Front-end. A ideia central é manter a página utilizável enquanto uma ação aguarda tempo, rede ou resposta externa.
...
```js
async function buscarUsuarios() { /* fetch */ }
function renderizarUsuarios(lista) { /* DOM */ }
...
Refatore um evento grande em três funções menores.
