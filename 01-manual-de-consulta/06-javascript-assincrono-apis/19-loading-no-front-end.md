...
Este capítulo ensina dar feedback enquanto a resposta não chega no contexto de JavaScript vanilla para Front-end. A ideia central é manter a página utilizável enquanto uma ação aguarda tempo, rede ou resposta externa.
...
try { await carregarDados(); } finally { loading.hidden = true; }
```
...
Adicione e remova um texto de carregamento no momento correto.
