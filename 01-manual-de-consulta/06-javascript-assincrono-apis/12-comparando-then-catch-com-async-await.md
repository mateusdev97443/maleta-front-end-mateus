...
Este capítulo ensina escolher a forma mais legível para o caso no contexto de JavaScript vanilla para Front-end. A ideia central é manter a página utilizável enquanto uma ação aguarda tempo, rede ou resposta externa.
...
// ou
async function carregar() { const r = await fetch(url); }
```
...
Reescreva um mesmo fluxo nas duas formas e compare a clareza.
