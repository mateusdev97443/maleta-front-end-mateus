...
Este capítulo ensina buscar termo em dados carregados ou em endpoint simples no contexto de JavaScript vanilla para Front-end. A ideia central é manter a página utilizável enquanto uma ação aguarda tempo, rede ou resposta externa.
...
const resultado = usuarios.filter(u => u.name.toLowerCase().includes(termo));
```
...
Crie uma busca por nome usando dados já carregados.
